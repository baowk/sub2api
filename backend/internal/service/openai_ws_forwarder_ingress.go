...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	acquireTurnLease = func(turn int, preferred string, forcePreferredConn bool) (*openAIWSConnLease, error) {
		...
		if acquireErr != nil {
			...
			var dialErr *openAIWSDialError
			if errors.As(acquireErr, &dialErr) && dialErr != nil && s.shouldFailoverOpenAIWSErrorStatus(dialErr.StatusCode) {
				...
				_ = s.handleOpenAIAccountUpstreamError(ctx, account, dialErr.StatusCode, dialErr.ResponseHeaders, nil)
	...
	for {
		...
		if s.isOpenAIAccountRuntimeBlocked(account) {
			...
			return NewOpenAIWSClientCloseError(
				coderws.StatusTryAgainLater,
				"upstream account is temporarily unavailable, please retry later",
				nil,
			)
		}
